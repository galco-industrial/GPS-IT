...
public class gpsdbf3_1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        debug (4, "Shutdown bit is currently " + shutDown);
        ...
        debug (4, "Shutdown bit is now " + shutDown);
        ...
        //debug (0, "Debug Level is " + DEBUGLEVEL);
        ...
        work = request.getParameter("batch");                    
        if (work != null) {
        ...
        */
        ...
        try {
            ...
            //debug(2, "Extracting part numbers; SQL statement is " + queryString);
            ...
            if (rs3 != null) {
                //debug (0, "I got the Result Set; Now building options for Family Code '" + familyCode + "'");
                ...
                while (rs3.next() && !abortSwitch) {   
                    ...
                    familyCode =    rs3.getString("family_code").toUpperCase();
                    ...
                    if (!familyCode.equals(familyCodePrev)) {                           // Did family code just change?
                        //debug (0, "Family '" + familyCodePrev + "' changed to '" + familyCode + "'.");
                        ...
                        //debug (0, "Subfamily '" + subfamilyCodePrev + "' changed to '" + subfamilyCode + "'.");
                        finishOldSeries();    // Perform Finish old series
                        finishOldMfgr();      // Perform Finish old Mfg
                        finishOldSubfamily(); // Perform Finish old subfamily:
                        startNewSubfamily();  // Perform Start new subfamily:
                        ...
                        //debug (0, "Mfgr '" + mfgrCodePrev + "' changed to '" + mfgrCode + "'.");
                        finishOldSeries();    // Perform Finish old series
                        finishOldMfgr();      // Perform Finish old Mfg
                        startNewMfgr();       // Perform Start new Mfg
                        startNewSeries();     // Perform Start new series
                        ...
                        //debug (0, "Series '" + seriesCodePrev + "' changed to '" + seriesCode + "'.");
                        finishOldSeries();    // Perform Finish old series
                        startNewSeries();     // Perform Start new series
                    ...
                    //debug (0, " Looking up parm data for part number " + partNumber);
                    ...
                    if (rs1 != null) {
                        ...
                        while (rs1.next()) {
                            ...
                            if (ruleSetSubfamilyVectorMap[seqNum] > -1)  {               // FIX ME to work with global/local seqnums
                                ...
                                //debug (0, "     Set parm value for seq num " + seqNum + " to '" + parmValue + "'");
                            }    
                        }
                        rs1.close();
                        rs1 = null; 
                    ...
                    //debug (0, " Found " + i + " parm values for part number " + partNumber);
                        ...
                        for (i = 0; parmSeqNums[i] > 0; i++) {
                            ...
                            if (!parmValue.equals("")) { // if an option value is present
                                ...
                                if (ruleSetSubfamilyVectorMap[seqNum] > -1) {
                                    optionsArraysIndex = optionsArraysSeriesVectorMap[seqNum]; // find out which Subfamily options array it goes inside
                                    if (optionsArraysIndex == -1) { // if options array does not exist yet for this seqnum, build one 
                                        ...
                                        optionsArraysSeries.add(optionsArray); // Add it to the list of subfamily (local) options arrays for this series                                
                                    ...
                                    //debug (0, "   Added raw value '" + parmValue + "' for Subfamily seq num " + seqNum + " to Series local options Array object");
                                }
                                ...
                                if (doGlobal) {  // This is currently disabled 
                                    if (ruleSetFamilyVectorMap[seqNum] > -1) {
                                        ...
                                        if (optionsArraysIndex == -1) { // if options array does not exist yet for this seqnum, build one 
                                            ...
                                            optionsArraysSeriesG.add(optionsArray); // Add it to the list of subfamily options arrays                                
                                        ...
                                        optionsArray.addOption(parmValue); // Add the parm value                             
                                        //debug (10, "   Added raw value '" + parmValue + "' for Family seq num " + seqNum + " to Series Global options Array object");
                                    }
                                }
                ...
                //debug (0, "EOF on part numbers result set.");
                if (!abortSwitch) {
                    finishOldSeries();    // Perform Finish old series
                    finishOldMfgr();      // Perform Finish old Mfg
                    finishOldSubfamily(); // Perform Finish old subfamily:
    ...
    private boolean checkForShutdown() {
        //debug (4, "Chceking shut down bit; shutdown = " + shutDown);
        if (shutDown) {
            abortSwitch = true;
            shutDown = false;
        }
    ...
    private void copySubfamilyOptionsToFamily() {
        ...
        for (i = 0; i < imax; i++) {
           ...
           for (j = 0; j < jmax; j++) {
               ...
               //debug (10, "   Added raw value '" + parmValue + "' for Family level Subfamily seq num " + workSeqNum + " to Family options Array object");
           } // end for
        } // end for
    ...
    private void copyMfgrOptionsToSubfamily() {
        ...
        for (i = 0; i < imax; i++) {
           ...
           for (j = 0; j < jmax; j++) {
               ...
               //debug (0, "   Added raw value '" + parmValue + "' for Series seq num " + workSeqNum + " to Mfgr options Array object");
           } // end for
        } // end for
    ...
    private void copySeriesOptionsToMfgr() {
        ...
        for (i = 0; i < imax; i++) {
           ...
           workSeqNum = fromOptionsArray.getSeqNum();
           //debug (0, "   fromOptionsArray Index is " + i + "; Seqnum is " + workSeqNum);
           toOptionsArraysIndex = optionsArraysMfgrVectorMap[workSeqNum]; // find out which option array it goes to
           //debug (0, "   toOptionsArrayIndex is " + toOptionsArraysIndex + "; Seqnum is " + workSeqNum);
           if (toOptionsArraysIndex == -1) { // if options array does not exist yet for this seqnum, build one 
               toOptionsArray = new GPSopts();  
               //debug (0, "  I just created a new GPSopts object for the Mfgr options Array.");
               junk = toOptionsArray.init(conn1, familyCodePrev, subfamilyCodePrev, workSeqNum);
               //debug (0, "  I just initialized the new GPSopts object.");
               ...
               //debug (0, "  The new GPSopts object Index is " + toOptionsArraysIndex);
               optionsArraysMfgrVectorMap[workSeqNum] = toOptionsArraysIndex; // update the vector map 
               //debug (0, "  The Mfgr vector map for seq num " + workSeqNum + " has been set to " + toOptionsArraysIndex);
           } else {
               toOptionsArray = optionsArraysMfgr.get(toOptionsArraysIndex); 
           }
           jmax = fromOptionsArray.getOptionListSize();
           //debug (0, "   Copying " + jmax + " Series parms to Mfgr options Array object");
           for (j = 0; j < jmax; j++) {
               parmValue = fromOptionsArray.getOptionListValue(j);
               //debug (0, "   Attempting to add value '" + parmValue + "' for entry index " + j);
               toOptionsArray.addOption(parmValue); // Add the parm value to the upper level array                             
               //debug (0, "   Added raw value #" + j + "  value '" + parmValue + "' for Subfamily level Series seq num " + workSeqNum + " to Mfgr options Array object");
           } // end for
        } // end for
    ...
    private boolean deleteAllFamilyOptionsFromDatabase(String aFamilyCode) {              // ECP-2
        String bSubfamilyCode;
        String bFamilyCode;
        String bMfgrCode;
        String bSignature;
        boolean junk;
        ...
        queryString =  "SELECT subfamily_code, parent_mfg_code ";
        queryString += " FROM pub.catalogitem";
        queryString += " WHERE family_code = '" + aFamilyCode + "'";
        queryString += " AND list_type = 'Catalog' ";
        queryString += " ORDER BY subfamily_code, parent_mfg_code";
        bFamilyCode = fcTestPrefix + aFamilyCode;
        try {
            deleteOptionHeaders(bFamilyCode);  // First delete the option list index headers
            rs3 = conn3.runQuery(queryString);
            if (rs3 != null) {
                //debug (0, "(852) Result set is not null.");
                while (rs3.next()) {
                    bSubfamilyCode = rs3.getString("subfamily_code");
                    bMfgrCode = rs3.getString("parent_mfg_code");
                    bSignature = bSubfamilyCode + "/" + bMfgrCode;
                    //debug (0, "bSubfamilyCode is " + bSubfamilyCode);
                    //debug (0, "prevCode is " + prevCode);
                    if (!bSignature.equals(prevSignature)) {
                        prevSignature = bSignature;
                        //debug (0, "Attempting to delete options for subfamily/mfgr code " + bSignature);
                        if (rc) {
                            rc = deleteAllSubfamilyMfgrOptions(bFamilyCode, bSubfamilyCode, bMfgrCode); // family/subfamily/mfg level
                            rc = deleteAllSubfamilyMfgrOptions(bFamilyCode, "*", bMfgrCode);            // family/*/mfg level
                        }
                        //debug (0, "Attempt completed; return code was " + rc);
                    }
                    if (!bSubfamilyCode.equals(prevCode)) {
                        prevCode = bSubfamilyCode;
                        //debug (0, "Attempting to delete options for subfamily level code " + bSubfamilyCode);
                        if (rc) {
                            rc = deleteAllSubfamilyMfgrOptions(bFamilyCode, bSubfamilyCode, "*");  // family/subfamily/* level
                        }
                        //debug (0, "Attempt completed; return code was " + rc);
                    }
                }
                rc = deleteAllSubfamilyOptions(bFamilyCode, "*");  // family/*/* level
                rs3.close();
                rs3 = null; 
                conn3.closeStatement();
                rc = deleteOrphanedFamilyJunk(bFamilyCode);
            } else {
                //debug (0, "Result set is null.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            closeAll();
    ...
    private boolean deleteAllSubfamilyOptions(String aFamilyCode, String aSubfamilyCode) {              // ECP-2
        ...
        //debug (0, "Attempting to delete option lists for family/subfamily: '" 
        //            + aFamilyCode + "/" + aSubfamilyCode + "'...");
    ...
    private boolean deleteAllSubfamilyMfgrOptions(String aFamilyCode, String aSubfamilyCode, String aMfgrCode) {              // ECP-2
        boolean rc;
        String message = "";
        String SQLCommand = "";
    ...
    private boolean deleteOptionHeaders(String aFamilyCode) {
        ...
        //debug (0, "Attempting to delete option list headers for family: " 
        //        + aFamilyCode );
        SQLCommand = "DELETE FROM pub.ps_option_lists";
        SQLCommand += " WHERE family_code = '" + aFamilyCode + "'";
    ...
    private void doCleanUp() {
        ...
        try {
            ...
            /////////////////////////////////////////////////////////////////////////////
            // Now we will Process the Result Set and                                  //
            // delete all options that have been erphaned                              //
            /////////////////////////////////////////////////////////////////////////////
    ...
    private void finishOldSeries() {
        ...
        while (optionsArraysSeries.size() > 0) {
             ...
             if (doMfg && doSeries ) {
                 //debug (0, "Writing options for Mfg/Series/SeqNum " 
        ...
        if (doGlobal) {  // Disabled for now
            ...
            //debug (0, "  Completed Series '" + seriesCodePrev + "'.");
        }
        // Begin ECP-5
        if (sqlHeaderEnabled) {
            int sqlHeaderIndex = 0;
    ...
    private void loadFamilyRuleSets(String aFamilyCode) {
        int i;
    ...
    private void loadSubfamilyRuleSets() {
        ...
        //debug (0, "Found " + fieldSetSubfamily.size() + " subfamily rulesets for family code " 
        ...
        //debug (0, SERVLET_NAME + " found " + ruleSetSubfamilyVectorMapCount + " Subfamily Search field ruleSets. ");
    }
    
    private void startNewFamily() {
        familyCodePrev = familyCode;
        ...
        if (!checkForShutdown()) {
            //debug (0, "****************************");
            debug (2, "Processing part numbers found for family code '" + familyCode +"' in catalogitem table.");
            //debug (0, "****************************");
            if (doGlobal) {
                loadFamilyRuleSets(familyCode);
            } 
            if (!deleteAllFamilyOptionsFromDatabase(familyCode)) {
                closeAll();
                done();
                System.out.println(SERVLET_NAME + " Error #1002 deleting ps_options from database; Aborting...");  
            }
        }
    }    
    ...
    private void startNewMfgr() {
        ...
        // Actually, we should be able to use the family snd subfamily vector maps here to check the seq nums....
        for (int i = 0; i < 100; i++) {
            //ruleSetMfgrVectorMap[i] = -1; // initially vector map is empty;  index i = seq num
            optionsArraysMfgrVectorMap[i] = -1;
            optionsArraysMfgrGVectorMap[i] = -1;
        }
    }
