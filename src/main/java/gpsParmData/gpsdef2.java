 ...
 * gpsdef2.java
 *
 * Created on October 10, 2006, 2:19 PM
 */
 ...
 * @version 1.5.00
 */
public class gpsdef2 extends HttpServlet {
	...
	work = request.getParameter("validation");
        ...
        try {
            ...
            series.clear();
            series.add( "\"\",\"Please select a Subcategory first\"" ); 
            if (!catCode.equals("All Categories") && j == 0) {
                conn.close();
                request.setAttribute("message", "Module " + SERVLET_NAME + " could not find any Series codes.");
                view = request.getRequestDispatcher("showMessage.jsp");
                view.forward(request,response);
                return;
            }
