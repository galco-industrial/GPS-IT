 ...
 *
 *
 * I export data rom the part table and parametric data if present.
 ...
 *
 */
public class gpsdef4 extends HttpServlet {
...
protected void processRequest(HttpServletRequest request, HttpServletResponse response)
    throws ServletException, IOException {
        ...
        try {
            ...
            for (f = 0; f < fieldSet.count(); f++) {
                ...
                // If we have a parm field that holds a picture name, remember the first field # here
                if (pictureFieldSeqNum == 0) {
                    if (tempDataType.equals("S")) {
                        if (rules[f].getImageType().equals("P")) {
                            pictureFieldSeqNum = rules[f].getSeqNum();
                        }
                    }
                }
            ...
            queryString += " stocked, obsolete, kit, misc_log1";
            queryString += " FROM pub.part";
            if (catCode.equals("*")) {
                queryString += " WHERE family_code = '" + famCode + "'";
                queryString += " AND subfamily_code = '" + subfamCode + "'";
            } else {
            ...
            if (!subcatCode.equals("*")) {  // this is really the manufacturer code
            ...
            //printHeaderC = ",\"Description\",\"Description2\",\"Description3\",\"Cat\",\"Picture\",\"[Location/Bin#]=Qty\"";
            printHeaderC = ",\"Description\",\"Description2\",\"Description3\",\"Cat\","
                           + "\"Obs\",\"Kit\",\"Stkd\",\"DoNotSell\",\"Picture\",\"[Location/Bin#]=Qty\"";
            printHeader = printHeaderA + printHeaderB + printHeaderC;
            ...
            while(rs.next()) {
                ...
                printLineC += sep2 + grab(rs, "sales_cat");
                printLineC += sep2 + grabYN(rs, "obsolete");
                printLineC += sep2 + grabYN(rs, "kit");
                printLineC += sep2 + grabYN(rs, "stocked");
                printLineC += sep2 + grabYN(rs, "misc_log1");
                printLineC += sep2 + GPSpart.getPartImageStatus(conn, conn3, partNumEdited, partSubcat, partSeries, pictureFieldSeqNum, famCode, subfamCode);
                printLineC += sep2 + GPSpart.getInStock(conn, partNumRaw) + sep1;

                printLine = printLineA + printLineB + printLineC;
                out.println(printLine);
                j++;
            }
            partNums.add(Integer.toString(j) + " part numbers found.");
            partNums.add("Export complete.");
            out.close();
            statement = rs.getStatement();
            rs.close();
            statement.close();
            ...
            view = request.getRequestDispatcher("gpsdef2.jsp");
            view.forward(request,response);  
    ...
    private String grabYN(ResultSet r, String columnName)
        throws SQLException {
        try {
            String result = r.getString(columnName).trim();
            if (result.equals("0")) {result = "No";}
            if (result.equals("1")) {result = "Yes";}
            return EditText.toDoubleQuote(result);
        }
        catch (SQLException e) {
            e.printStackTrace();
            return "\"error\"";
    ...
    public String getServletInfo() {
        return "I am the dudette that generates a worksheet for part numbers in a specified Category/SUbcatergory/Series.";
    }
    // </editor-fold>
}
