 ...
 * gpsoaf2.java
 *
 * Created on October 28, 2010, 4:01 PM
 */
 ...
 *
 */
public class gpsoaf2 extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        int j = 0;
        int k = 0;
        String imagePath = "/var/www/www.galco.com/htdocs/images/";
        //String imagePath = fsep + fsep + "alpha" + fsep + "webcontent" + fsep + "images" + fsep;
        ...
        try {
            ...
            fileName = "imageaudit." + auditUserID.toLowerCase() + "." + removeSlashes(auditDate) + "." + hhmmss;
            fExt = ".txt";
            ...
            if (baseDir.exists() && baseDir.isDirectory()) {
                                ...
                                for (i1 = 0; i1 < familyContents.length; i1++) {  // iterate through elements in the family dir
                                    ...
                                    if (workElement.isDirectory()) {
                                        ...
                                        if (subfamilyContents == null || (subfamilyContents != null && subfamilyContents.length == 0)) {
                                            logItem = "      WARNING! - Subfamily Code Directory '" + familyDir.getName() + fsep + subfamilyDir.getName() + "' is empty.";
                                            if (logLevel > 1) {out.println(logItem);}
                                            debug (debugLevel, 2, logItem); 
                                        } else {
                                            for (i2 = 0; i2 < subfamilyContents.length; i2++) {  // iterate through elements in the subfamily dir
                                                ...
                                                if (workElement.isDirectory()) {
                                                    logItem = "      ERROR! Subfamily element " + (i2 + 1) 
                                                            + " " + familyDir.getName() + fsep + subfamilyDir.getName()
                                                    ...
                                                    if (logLevel > 2) {out.println(logItem);}
                                                    debug (debugLevel, 2, logItem); 
                                                    selectBoxNames = getSelectBoxNames(conn, familyDir.getName(), subfamilyDir.getName(), subfamilyFile.getName());
                                                    if (selectBoxNames != null) {
                                                     for (k = 0; k < selectBoxNames.length; k++) {
                                                      selectBoxName = selectBoxNames[k];
                                                      if (selectBoxName == null) {
                                                          selectBoxName = "";
                                                      }
                                                      if (selectBoxName.equals("") && k > 0) {break;}
                                                      if (selectBoxName.equals("")) {
                                                        logItem = "      WARNING! "
                                                                + familyDir.getName() + fsep + subfamilyDir.getName()
                                                                + fsep + subfamilyFile.getName() + " is not referenced in an image select box.";
                                                        if (logLevel > 1) {out.println(logItem);}
                                                        debug (debugLevel, 2, logItem); 
                                                      } else {
                                                        ...
                                                        if (mCodes == null) {
                                                            ...
                                                            if (mCodes[0] != null && mCodes[0].equals("No Rule")) {
                                                                logItem = "    WARNING! No RuleSet was found using select box '"
                                                                    ...
                                                                    + familyDir.getName() + fsep +  familyFile.getName();
                                                                if (logLevel > 1) {out.println(logItem);}
                                                                debug (debugLevel, 2, logItem); 
                                                            } else {
                                                                for (j = 0; j < mCodes.length; j++) {
                                                                    ...
                                                                    if (mfgrImage.exists() && mfgrImage.isFile()) {
                                                                        logItem = "        ERROR! File '" 
                                                                                + familyDir.getName() + fsep + subfamilyDir.getName() + fsep
                                        ...
                                        logItem = "  Family directory element " + (i1 + 1) 
                                                ...
                                                + familyDir.getName() + fsep + familyFile.getName() + "', length: "
                                                + familyFile.length();
                                        if (logLevel > 2) {out.println(logItem);}
                                        debug (debugLevel, 1, logItem); 
                                        selectBoxNames = getSelectBoxNames(conn, familyDir.getName(), "*", familyFile.getName());
                                        if (selectBoxNames != null) {
                                         for (k = 0; k < selectBoxNames.length; k++) {
                                          selectBoxName = selectBoxNames[k];
                                          if (selectBoxName == null) {
                                          ...
                                          if (selectBoxName.equals("") && k > 0) {break;}
                                          if (selectBoxName.equals("")) {
                                            logItem = "    WARNING! " + familyDir.getName() + fsep +  familyFile.getName() 
                                            + " is not referenced in a global image select box.";
                                            if (logLevel > 1) {out.println(logItem);}
                                            debug (debugLevel, 2, logItem); 
                                          } else {
                                            logItem = "    Processing select box '"
                                                        + selectBoxName + "' containing image file named "    
                                                        + familyDir.getName() + fsep +  familyFile.getName();
                                            if (logLevel > 2) {out.println(logItem);}
                                            debug (debugLevel, 4, logItem); 
                ...
                logItem = "ERROR! Base directory for images" + fsep + "catalog" + fsep + "sb" + fsep + " was not found.";
                if (logLevel > 0) {out.println(logItem);}
                debug (debugLevel, 1, logItem); 
            }
            ...
            debug (debugLevel, 1, logItem); 
            out.close();
            request.setAttribute("statusMessage", "Log written to " + outFileName);
            view = request.getRequestDispatcher("gpsoaf1.jsp");
            view.forward(request,response);
            conn.closeStatement();
            conn.close();
