 ...
 * gpsomf3.java
 *
 * Created on March 6, 2007, 1:36 PM
 */
 ...
 *
 * *
 * I get info to prepare to modify a selected Select Box Option
 ** 
 * 09/01/2007   DES
 ...
 *
 */
public class gpsomf3 extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        // Note that we ignore if enableToolTips changed here
        ...
        try {
            ...
            if (selectBox.open(conn, familyCode, subfamilyCode, selectBoxName) > -1) {
                ...
                RequestDispatcher view = request.getRequestDispatcher("gpsomf3.jsp");
                view.forward(request,response);
            } else {
                request.setAttribute("message", "Module " + SERVLET_NAME + " failed to obtain the Select Box Info.");
                RequestDispatcher view = request.getRequestDispatcher("showMessage.jsp");
                view.forward(request,response);
            }
        } catch (Exception e) {
