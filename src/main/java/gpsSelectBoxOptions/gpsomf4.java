 ...
 * gpsomf4.java
 *
 * Created on March 6, 2007, 3:13 PM
 */
 ...
 * make the modifications to the Select Box Option in WDS.
 ** 
 * 09/01/2007   DES
 ...
 *
 */
public class gpsomf4 extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            debug (debugLevel, 4, uStamp + " Attempting to modify Option " + oldOptionText + " in Select Box " 
                    + familyCode + "/" + subfamilyCode + "/" + selectBoxName);
            SQLCommand = "UPDATE pub.ps_select_boxes";
            SQLCommand += " SET option_text = '" + optionText +"'";
            SQLCommand += ", option_image = '" + optionImage + "'";
            SQLCommand += ", option_value1 = '" + optionValue1 + "'";
            SQLCommand += ", option_value2 = '" + optionValue1 + "'";
            SQLCommand += ", option_index = " + displayOrder;
            SQLCommand += ", option_dflt = " + optionDefaultCode;
            SQLCommand += " WHERE select_box_name = '" + selectBoxName + "'";
            SQLCommand += " AND family_code = '" + familyCode + "'";
            SQLCommand += " AND subfamily_code = '" + subfamilyCode + "'";
            SQLCommand += " AND option_index = " + oldDisplayOrder;
            debug (debugLevel, 4, uStamp + " SQL Command is " + SQLCommand);
            completedOK = conn.runUpdate(SQLCommand);
            message = " Option " + oldOptionText + " in " + selectBoxName + " was";
            if (!completedOK) {
                message += " NOT";
            }
            message += " modified successfully for family code " + familyCode
            ...
            // Invoke our JSP
            RequestDispatcher view = request.getRequestDispatcher("gpsomf2.jsp");
            view.forward(request,response);
        } catch (Exception e) {
            sWork = uStamp + " unexpected error " + e;
            debug (debugLevel, 0, sWork);
